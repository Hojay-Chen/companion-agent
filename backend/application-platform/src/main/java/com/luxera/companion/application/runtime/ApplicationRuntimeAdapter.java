...
/**
 * 进程内应用对 {@link ApplicationRuntimePort} 的实现 —— 数字人在进程内的唯一应用入口。
 *
 * <p>它把若干 {@link LocalApplicationProvider} 聚合成一个 LAP 端口, 执行仍走
 * {@link DefaultActionsRuntime}(权限决策 + 审计日志), 因此 <b>R3 是纯行为保持</b>:
 * 没有权限语义变化, 没有新的持久化, 也没有真实的幂等与 CAS。
 *
 * <p>原先它叫 {@code LocalApplicationRuntimeAdapter} 并住在 digital-human-platform 里 ——
 * 那是 R2 的过渡形态。R4 起由 {@code ActionGateway} 实现本端口(真实幂等 + Resource CAS +
 * 归属校验), 本类随之删除; {@code ApplicationRuntimeImplementationArchitectureTest} 会按类名
 * 断言它不存在, 防止它被悄悄加回来。
 */
@Slf4j
@Service
public class ApplicationRuntimeAdapter implements ApplicationRuntimePort {

    private final DefaultActionsRuntime actions;
    private final List<LocalApplicationProvider> providers;
    private final ObjectMapper objectMapper;

    public ApplicationRuntimeAdapter(DefaultActionsRuntime actions,
                                     List<LocalApplicationProvider> providers,
                                     ObjectMapper objectMapper) {
        this.actions = actions;
        this.providers = List.copyOf(providers);
        this.objectMapper = objectMapper;
        log.info("[ApplicationRuntime] 已挂载 {} 个进程内应用: {}", providers.size(),
