...
class MessagePipelineTest {
    ...
    void setUp() {
        ...
        Conversation conv = conversationService.create(userId, companionId, "测试会话", "小满");
    ...
    private MessageView send(String content) {
        com.luxera.companion.agent.PerceptionEngine.Perception p = perceptionEngine.perceive(content);
        Message m = conversationService.addMessage(conversationId, "user", content,
                p.intent(), p.emotion(), p.topic(), false, null, null, null, null);
        return com.luxera.companion.conversation.MessageViews.toView(m);
    }
    ...
    void daytimeEmotionalMessageIsNoticed() {
        MessageView m = send("我今天好难过,项目被砍了,有点撑不住");
        MessagePipeline.PipelineResult r = pipeline.process(userId, companionId, conversationId,
    ...
    void emotionStateIsUpdatedViaReducer() {
        MessageView m = send("你怎么这么烦,跟你说话真没意思");
    ...
    void tracesAreRecordedForAgents() {
        MessageView m = send("我今天好难过,项目被砍了");
    ...
    void replyPathProducesGeneratedMessage() {
        ...
        var outcome = runtime.generate(userId, companionId, conversationId, m.getId(),
                m.getContent(),
                com.luxera.companion.conversation.MessageViews.toViews(
                        conversationService.recentMessages(conversationId, 30)),
                null, decision, hint);
        assertNotNull(outcome.reply());
        assertFalse(outcome.reply().isBlank(), "生成回复不应为空");
        // 生成后模拟控制器落库(addMessage), 验证回复可写入会话
        Message sent = conversationService.addMessage(conversationId, "companion", outcome.reply(),
