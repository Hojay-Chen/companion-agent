...
import com.luxera.companion.conversation.MessageViews;
...
class AgentReadAllTest {
    ...
    private MessageView userMessage(String content) {
        Message m = new Message();
        m.setConversationId(conversationId);
        m.setSenderType("user");
        m.setContent(content);
        m.setDeliveryStatus(MessageLifecycle.DELIVERED);
        return MessageViews.toView(messageRepository.save(m));
    }

    @Test
    void processingLastBatchMarksEntireConversationRead() {
        // 用户连发两条(分批到达): 第一条先到(未读), 第二条稍后到
        MessageView first = userMessage("在吗?");
        MessageView second = userMessage("怎么一直不回我");
    ...
    void deferredStillMarksEntireConversationRead() {
        ...
        MessageView first = userMessage("我最近被裁员了,心里很难受");
        MessageView second = userMessage("想跟你说说");
