...
public class ApplicationRegistry {
    ...
    private final DhApplicationRepository repository;

    public ApplicationRegistry(DhApplicationRepository repository) {
    ...
    private void seedIfMissing(String code, String name, String manifest) {
        try {
            if (repository.findByCode(code).isEmpty()) {
                DhApplication app = new DhApplication();
                app.setCode(code);
                app.setName(name);
    ...
    /** 列出所有 ACTIVE 应用 */
    public java.util.List<DhApplication> list() {
        return repository.findAll();
    }

    /** 按 code 查找 */
    public java.util.Optional<DhApplication> byCode(String code) {
