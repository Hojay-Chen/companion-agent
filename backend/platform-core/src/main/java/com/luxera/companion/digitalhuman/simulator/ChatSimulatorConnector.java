...
public class ChatSimulatorConnector {
    ...
    public DeviceConnection connect(String companionId, String deviceId, String accountId, String secret) {
        ...
        try {
            ...
            // 4. 注册 SimulatorSession(让 COMMAND 路径能 scope 校验)
            sessionRegistry.open(accountId, Set.of(
    ...
    public void disconnect(String companionId) {
        ...
        if (dc != null) {
            dc.close();
            sessionRegistry.disconnect(dc.accountId);
    ...
    /**
     * 用 device secret 向 chat 平台刷新短期访问 token。
     * 单进程过渡期: 直接调 SimulatorPairingService(同在 JVM);
     * 双进程后: 通过 HTTPS POST /api/simulator/refresh-token。
     */
    private String refreshToken(String deviceId, String secret) {
        try {
            // 单进程过渡: 直接调用
            var pairingService = com.luxera.companion.ApplicationContextProvider
                    .getBean(com.luxera.companion.simulator.server.SimulatorPairingService.class);
            return pairingService.refreshBySecret(deviceId, secret);
        } catch (Exception e) {
            log.warn("[ChatSimulatorConnector] token 刷新失败: {}", e.getMessage());
            return null;
        }
    }
