 ...
 *
 * <p><b>R2 起它同时是 LAP 的"应用侧"</b>: 实现 {@link LocalApplicationProvider},
 * 于是数字人只通过 {@code ApplicationRuntimePort} 看见它 —— 棋盘长什么样、轮到谁、
 * 下一步怎么走才聪明, 全部留在这个类里, 不再渗进 {@code AgentRuntime}。
 *
 * <p>动作:
...
public class TicTacToeApplicationAdapter implements LocalApplicationProvider {
    ...
    void registerActions() {
        // 旧 runtime 的 ActionDescriptor 仍用 digitalhuman.application.domain 下的枚举;
        // R3 搬进 application-platform 后这两个枚举删除, 统一到 contracts.application.*。
        actionRuntime.register(ACTION_CREATE, this::create, ActionDescriptor.of(
                ACTION_CREATE, APP_CODE, "开局(真人先手)",
                com.luxera.companion.digitalhuman.application.domain.PermissionLevel.WRITE,
                com.luxera.companion.digitalhuman.application.domain.RiskLevel.LOW));
        actionRuntime.register(ACTION_STATE, this::readState, ActionDescriptor.of(
                ACTION_STATE, APP_CODE, "读取当前棋盘局面",
                com.luxera.companion.digitalhuman.application.domain.PermissionLevel.READ,
                com.luxera.companion.digitalhuman.application.domain.RiskLevel.NONE));
        actionRuntime.register(ACTION_MAKE_MOVE, this::makeMove, ActionDescriptor.of(
                ACTION_MAKE_MOVE, APP_CODE, "落子",
                com.luxera.companion.digitalhuman.application.domain.PermissionLevel.WRITE,
                com.luxera.companion.digitalhuman.application.domain.RiskLevel.LOW));
        actionRuntime.register(ACTION_SURRENDER, this::surrender, ActionDescriptor.of(
                ACTION_SURRENDER, APP_CODE, "认输/投降",
                com.luxera.companion.digitalhuman.application.domain.PermissionLevel.WRITE,
                com.luxera.companion.digitalhuman.application.domain.RiskLevel.LOW));
    ...
    private ActionRuntime.ActionResult makeMove(String actionId, Map<String, Object> input,
        ...
        // 真人路径显式传 "user"(见 ApplicationGameController / TicTacToeGameService 的既有调用)。
    ...
    private ActionRuntime.ActionResult surrender(String actionId, Map<String, Object> input,
        ...
        return ActionRuntime.ActionResult.success(
                Map.of("roomId", s.getRoomId(), "status", s.getStatus(), "uri", uriOf(roomId)),
